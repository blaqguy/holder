...
if [[ -z "${GITHUB_ACCESS_TOKEN}" || -z "${GITHUB_ACTIONS_RUNNER_CONTEXT}" || -z "${RUNNER_LABELS}" ]]; then
...
./config.sh --url "${GITHUB_ACTIONS_RUNNER_CONTEXT}" --token "${RUNNER_TOKEN}" --work "${RUNNER_WORKDIR}" --labels "${RUNNER_LABELS}"
./run.sh